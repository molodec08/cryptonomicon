      ...
      <section>
        <div class="flex">
          <div class="max-w-xs">
            ...
            >
              <span
                v-for="(hint, key) in hintsList"
                :key="key"
                @click="clickHint(hint)"
  ...
  methods: {
    ...
    findTicker() {
      ...
      if (!this.tickers.find(t => t.name === ticker)) {
        ...
        this.hintsList = [];
        for (let f in this.coinList) {
          if (
            f
              .split("")
              .slice(0, ticker.length)
              .join("")
              .toUpperCase() === ticker &&
            ticker !== ""
          ) {
            this.hintsList.push(f);
          }
        }
        this.hintsList = this.hintsList.slice(0, 4);
      } else {
